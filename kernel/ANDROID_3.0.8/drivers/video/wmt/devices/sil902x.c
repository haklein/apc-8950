...
sil902x_ko_ops_t *sil902x_ko;
vout_dev_t sil902x_vout_dev_ops;
...
static char ReadByte(unsigned int index)
{
	...
	vpp_i2c_read(VPP_DVI_I2C_ID,SIL902X_ADDR,index,&temp,1);
	return temp;
}

static void WriteByte(unsigned int index,char data)
{
	vpp_i2c_write(VPP_DVI_I2C_ID,SIL902X_ADDR,index,&data,1);
}

static void WriteBlock(unsigned int index,char *buf,int len)
{
	vpp_i2c_write(VPP_DVI_I2C_ID,SIL902X_ADDR,index,buf,len);
}
...
static DECLARE_MUTEX(sil902x_sem);
#else
static DEFINE_SEMAPHORE(sil902x_sem);
#endif
#endif

void sil902x_lock(void)
{
#ifdef __KERNEL__
	down(&sil902x_sem);
#else
...
int sil902x_CP_get_KSV(void)
{
	...
	vpp_i2c_read(VPP_DVI_I2C_ID,0x74,0x41,&temp,1);
    KeyCount = (temp & 0x7F) * 5;
	if (KeyCount != 0){
		vpp_i2c_read(VPP_DVI_I2C_ID,0x74,0x43,KSV_Array,KeyCount);
	}
#if 1
{
	int i;
...
EXPORT_SYMBOL(sil902x_enable_access);
...
* \retval  None
*/
void sil902x_do_read_edid(int index,char *buf)
{
	char int_bk;
	char reg;
	...
	if( index < 0x100 ){
		vpp_i2c_read(VPP_DVI_I2C_ID,0xA0,index,(char *)buf,128);
	}
	else {
		vpp_i2c_enhanced_ddc_read(0xA0,index-0x100,(char *)buf,128);
	}
...
sil902x_mode_t sil902x_get_vmode(vout_info_t *info)
{
	...
	mode = VMODE_MAX;
	sil902x_vic = HDMI_UNKNOW;	
	switch( info->resx ){
		case 640:
    		mode = VMODE_640x480;
			sil902x_vic = HDMI_640x480p60_4x3;
			break;
		case 720:
			if( info->resy == 480 ){
				if( pixclock == 27000000 ){
					mode = VMODE_480i60;
					sil902x_vic = HDMI_1440x480i60_16x9;
				}
				else {
					mode = VMODE_480p60;
					sil902x_vic = HDMI_720x480p60_16x9;
				}
			}
			if( info->resy == 576 ){
				if( pixclock == 27000000 ){
				    mode = VMODE_576p50;
					sil902x_vic = HDMI_720x576p50_16x9;
				}
				else {
				    mode = VMODE_576i50;
					sil902x_vic = HDMI_1440x576i50_16x9;
				}
			}
			break;
		case 800:
		...
		case 1280:
			if( info->resy == 720 ){
				if( pixclock == 74250050 ){
					mode = VMODE_720p50;
					sil902x_vic = HDMI_1280x720p50_16x9;
				}
				else {
					mode = VMODE_720p60;
					sil902x_vic = HDMI_1280x720p60_16x9;
				}
			}
			if( info->resy == 1024 ){
    	...
    	case 1920:
			if( info->resy == 1080 ){			
				switch( pixclock ){
					case 74250060:
					    mode = VMODE_1080i60;
						sil902x_vic = HDMI_1920x1080i60_16x9;
						break;
				    case 74250050:
				    	mode = VMODE_1080i50;
						sil902x_vic = HDMI_1920x1080i50_16x9;
						break;
					case 148500050:
				    	mode = VMODE_1080p50;
						sil902x_vic = HDMI_1920x1080p50_16x9;
						break;
					default:
					case 148500000:
				    	mode = VMODE_1080p60;
						sil902x_vic = HDMI_1920x1080p60_16x9;
						break;
				}
			}
			if( info->resy == 1200 ){
	...
	if( mode == VMODE_MAX ){
		DPRINT("[sil902x] *E* not support %dx%d,%d,vic %d\n",info->resx,info->resy,pixclock,sil902x_vic);
	}
	return mode;
...
EXPORT_SYMBOL(sil902x_get_audio);

int sil902x_get_audio_i2s(void)
{
	return ( vpp_get_hdmi_spdif() )? 0:1;
}
EXPORT_SYMBOL(sil902x_get_audio_i2s);

int sil902x_get_audio_channel(void)
{
	return sil902x_audio_channel;
}
EXPORT_SYMBOL(sil902x_get_audio_channel);

int sil902x_get_cp_support(void)
{
	return g_vpp.hdmi_cp_enable;
}
EXPORT_SYMBOL(sil902x_get_cp_support);
...
//	sil902x_clr_interrupt();
	...
	if( hotplug ){
		if (plugin){
			option = vout_get_edid_option(VPP_VOUT_NUM_DVI);
...
void sil902x_init_module(int ko)
{
	DPRINT("[SIL902X] %s 0x%x\n",__FUNCTION__,ko);
	if( sil902x_ko )
		return;
	sil902x_ko = (sil902x_ko_ops_t *) ko;
}
EXPORT_SYMBOL(sil902x_init_module);
...
void sil902x_poll(void)
{
	...
	if( sil902x_ko ){
		int status;
		char link_level;
		int arg[SIL902X_STATUS_ARG_NUM];
		static int pre_arg[SIL902X_STATUS_ARG_NUM];
		int i;

		status = sil902x_ko->poll();
		sil902x_ko->get_status(arg);
		status = 0;
		link_level = arg[1];
		for(i=0;i<SIL902X_STATUS_ARG_NUM;i++){
			if( arg[i] != pre_arg[i] ){
				switch(i){
					case 0: // plugin
						status |= 0x1;
		...
		if( status & 0x1 ){	// Plug status change
			int plugin;
			
			plugin = sil902x_check_plugin(0);
...
#endif
			DPRINT("[SIL902X] plug%s\n",(plugin)?"in":"out");
		}
		
		if( (vo->status & VPP_VOUT_STS_PLUGIN) && (status & 0x80) ){ // HDCP status change
			if( link_level == LOCAL_LINK_PROTECTION_SECURE ){
...
{
	ReadModifyWrite(0x1A,BIT3,(enable)?BIT3:0);
	return;
	if( enable ){
...
int sil902x_set_audio(vout_audio_t *arg)
{
    ...
    bk = ReadByte(0x1A);
	WriteByte(0x1A,0x18);	// restart
	WriteByte(0x27,sil902x_get_audio());		// Audio size, sampling frequency, channel number all refer to header 
		
	reg = (vpp_get_hdmi_spdif())? 0x40:0x80; 	// bit7:6	 01:SPDIF, 10:I2S
	if( arg->channel > 2 )
		reg |= BIT5;							// set layout bit to 1 for multi-channel support
	WriteByte(0x26,reg); 						// SPDIF, un-mute, refer header 
	WriteByte(0x1A,bk);	// restart
