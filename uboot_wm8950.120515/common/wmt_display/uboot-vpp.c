...
int wmt_graphic_init(void)
{
	...
	p_govrh->fb_p->fb.col_fmt = p_govrh2->fb_p->fb.col_fmt = VPP_UBOOT_COLFMT;
	g_vpp.alloc_framebuf = 0; // uboot_vpp_alloc_framebuffer;
	g_vpp.mb[0] = g_fb_phy;
	g_vpp.mb[1] = g_vpp.mb[0];
	vpp_init();
	vout_check_info((g_vpp.dual_display)? info->vo_mask:VPP_VOUT_ALL,&vout_info[0]);
	
#ifdef WMT_DUAL_DISPLAY_LOGO
	vfb_var.xres = 1920;
	vfb_var.yres = 1080;
#else
	vfb_var.xres = vout_info[0].resx;
	vfb_var.yres = vout_info[0].resy;
